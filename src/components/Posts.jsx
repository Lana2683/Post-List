...
class Posts extends PureComponent {
    ...
    render() {
        ...
        let filteredPosts = posts.filter(post => 
            post.body.indexOf(searchText)!==-1 || post.title.indexOf(searchText)!==-1);
...
onClickLike = (id) => {
    ...
    this.setState({likedArr})
}

onClickDisLike = (id) => {
    const { posts } = this.props;
    const dislikedArr = posts.map(post => {
        if(post.id === id){ 
